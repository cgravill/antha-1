...
func (vbln volumesByLiquidName) Summary(labEffects *effects.LaboratoryEffects, lhp *LHProperties) string {
...
func (lhp *LHProperties) GetComponents(labEffects *effects.LaboratoryEffects, cmps wtype.ComponentVector, ori wtype.ChannelOrientation, multi int, independent bool, legacyVolume bool) ([]ParallelTransfer, error) {
	...
	currCmps := cmps.Dup(labEffects.IDGenerator)
	...
	for !currCmps.IsEmpty() {
		...
		for i, src := range srcs {
			if !src.IsEmpty() {
				...
				match, err := wtype.MatchComponents(labEffects.IDGenerator, currCmps, src, independent, false)
