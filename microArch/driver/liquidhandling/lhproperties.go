...
func (lhp *LHProperties) dup(idGen *id.IDGenerator, keepIDs bool) *LHProperties {
	...
	r := NewLHProperties(idGen, lhp.Model, lhp.Mnfr, lhp.LHType, lhp.TipType, pos)
...
func NewLHProperties(idGen *id.IDGenerator, model, manufacturer string, lhtype LiquidHandlerLevel, tiptype TipType, positions map[string]*wtype.LHPosition) *LHProperties {
...
func (lhp *LHProperties) UpdateID(idGen *id.IDGenerator, currID string) (string, error) {
	if thing, ok := lhp.PlateLookup[currID]; !ok {
		...
		newObj := obj.Duplicate(idGen, false)
...
func (p *LHProperties) SaveUserPlates(idGen *id.IDGenerator) UserPlates {
	...
	for addr, plate := range p.Plates {
		if plate.IsUserAllocated() {
			up = append(up, UserPlate{Plate: plate.DupKeepIDs(idGen), Position: addr})
		}
	}
