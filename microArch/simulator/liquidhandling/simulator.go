...
func (self *VirtualLiquidHandler) getTargetPosition(idGen *id.IDGenerator, adaptorName string, channelIndex int, deckposition, platetype string, wc wtype.WellCoords, ref wtype.WellReference) (wtype.Coordinates3D, bool) {
...
func (self *VirtualLiquidHandler) Aspirate(volume []float64, overstroke []bool, head int, multi int,
	...
	for well, v := range volumeTakenByWell {
		...
		if volume.GreaterThan(well.CurrentVolume(self.idGen).PlusEpsilon()) {
			...
			self.AddErrorf("%s: taking %s from %s which contains only %s working plus %s residual volume (possibly a side effect of ANTHA-2704, try using a plate with a larger residual volume)",
				describe(), volume, well.GetName(), well.CurrentVolume(self.idGen), well.ResidualVolume())

		} else if volume.GreaterThan(well.CurrentWorkingVolume(self.idGen).PlusEpsilon()) {
			...
			self.AddWarningf("%s: taking %s from %s which contains only %s working volume, possible aspiration of residual (see ANTHA-2704)",
				describe(), volume, well.GetName(), well.CurrentWorkingVolume(self.idGen))
		}
	}
	...
	for _, i := range arg.channels {
		if wells[i] == nil {
			...
			tipVol := tip.CurrentVolume(self.idGen)
			tipVol.Add(aspVol)
			...
			} else if c, err := wells[i].RemoveVolume(self.idGen, aspVol); err != nil {
				self.AddErrorf("%s: unexpected well error \"%s\"", describe(), err.Error())
			} else {
				err := tip.AddComponent(self.idGen, c)
	...
	for _, well := range wells {
		if well != nil {
			well.RemoveCarry(self.idGen, self.properties.CarryVolume())
		}
	}
...
func (self *VirtualLiquidHandler) Dispense(volume []float64, blowout []bool, head int, multi int,
	...
	for _, i := range arg.channels {
		...
		if c, err := tip.RemoveVolume(self.idGen, v); err != nil {
			self.AddErrorf("%s: unexpected tip error \"%s\"", describe(), err.Error())
		} else if err := wells[i].AddComponent(self.idGen, c); err != nil {
			self.AddErrorf("%s: unexpected well error \"%s\"", describe(), err.Error())
		}
	}
...
func (self *VirtualLiquidHandler) Mix(head int, volume []float64, platetype []string, cycles []int,
	...
	for _, ch := range arg.channels {
		if tip := arg.adaptor.GetChannel(ch).GetTip(); !tip.IsEmpty() {
			...
			nonEmptyVolumes = append(nonEmptyVolumes, tip.CurrentVolume(self.idGen).MustInStringUnit("ul").RawValue())
		}
	}
