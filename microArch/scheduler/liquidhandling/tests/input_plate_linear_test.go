...
func summarize(m map[string]map[*wtype.LHPlate]int) map[string]map[wtype.PlateTypeName]int {
	r := make(map[string]map[wtype.PlateTypeName]int, len(m))

	for k, v := range m {
		mm := make(map[wtype.PlateTypeName]int, len(v))
...
func TestIPL1(t *testing.T) {
	testlab.WithTestLab(t, "", &testlab.TestElementCallbacks{
		Steps: func(lab *laboratory.Laboratory) error {
			// a few plates
			pcrplateSkirted, errA := lab.Inventory.Plates.NewPlate("pcrplate_skirted")
			DWST12, errB := lab.Inventory.Plates.NewPlate("DWST12")
			DSW96, errC := lab.Inventory.Plates.NewPlate("DSW96")
			SRWFB96, errD := lab.Inventory.Plates.NewPlate("SRWFB96")
			reservoir, errE := lab.Inventory.Plates.NewPlate("reservoir")
			if err := (utils.ErrorSlice{errA, errB, errC, errD, errE}).Pack(); err != nil {
				return err
			}
			...
			for _, testCase := range testCases {
				testlab.WithTestLab(t, "", &testlab.TestElementCallbacks{
					...
					Steps: func(lab *laboratory.Laboratory) error {
						...
						results, err := lh.ChoosePlateAssignments(lab.IDGenerator, testCase.Component_volumes, testCase.Plate_types, testCase.Weight_constraint)

						if _, fail := testCase.Expected["FAIL"]; fail {
							// routine should fail

							if err == nil {
								return fmt.Errorf("Expected failure, got %v", summarize(results))
							}
							...
							if err != nil {
								return fmt.Errorf("Expected %s got error %v", fmtIt(testCase.Expected), err.Error())
							}

							if !compare(results, testCase.Expected) {
								return fmt.Errorf("Expected: %s got %s", fmtIt(testCase.Expected), fmtIt(results))
							}
						}
						return nil
					},
