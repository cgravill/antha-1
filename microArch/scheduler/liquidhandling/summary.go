...
import (
	...
	simulator "github.com/Synthace/antha/microArch/simulator/liquidhandling"
...
// errors are returned if the json cannot be constructed or the result fails to validate
func SummarizeActions(idGen *id.IDGenerator, initialState *driver.LHProperties, itree *driver.ITree) ([]byte, error) {
	...
	vlh, err := simulator.NewVirtualLiquidHandler(idGen, initialState, settings)
	if err != nil {
		return nil, err
	}
	...
	for _, act := range acts {
		...
		case driver.TFB:
			...
			if action, err := newTransferAction(idGen, vlh, act, timer, cumulativeTime); err != nil {
				return nil, err
			} else {
				action.TimeEstimate = timeForAct.Seconds()
				cumulativeTime = time.Duration(action.CumulativeTimeEstimate * 1e9)
...
func newDeckSummary(idGen *id.IDGenerator, props *driver.LHProperties) *deckSummary {
	...
	for posName, pos := range props.Positions {
		if objID, ok := props.PosLookup[posName]; ok {
			positions[posName] = &deckPosition{
				...
				Item:     newItemSummary(idGen, props.PlateLookup[objID].(wtype.LHObject)),
			}
		} else {
			positions[posName] = &deckPosition{
				Position: newCoordinates3D(pos.Location),
...
func newItemSummary(idGen *id.IDGenerator, obj wtype.LHObject) *itemSummary {
		...
		for i, col := range o.Cols {
			...
			for j, well := range col {
				if !well.IsEmpty(idGen) {
					c[j] = newLiquidSummary(well.Contents(idGen))
				}
			}
			if len(c) > 0 {
				contents[i] = c
...
func newContentUpdate(idGen *id.IDGenerator, well *wtype.LHWell) *contentUpdate {
	return &contentUpdate{
		...
		NewContent: newLiquidSummary(well.Contents(idGen)),
	}
}
...
func newParallelTransfer(idGen *id.IDGenerator, vlh *simulator.VirtualLiquidHandler, tree *driver.ITree, timeEstimate, cumulativeTimeEstimate time.Duration) (*ParallelTransfer, error) {
	...
	for _, ch := range channels {
		...
		missing := wunit.SubtractVolumes(expectedVolume[sourceWells[ch]], sourceWells[ch].CurrentVolume(idGen))
		missing.DivideBy(float64(len(channelsInWell[sourceWells[ch]])))

		tfs := &transferSummary{
			From:              newContentUpdate(idGen, sourceWells[ch]),
			To:                []*contentUpdate{newContentUpdate(idGen, destWells[ch])}, // multi dispense will require multiple entries here
...
func newTransferAction(idGen *id.IDGenerator, vlh *simulator.VirtualLiquidHandler, act *driver.ITree, timer driver.LHTimer, cumulativeTimeEstimate time.Duration) (*TransferAction, error) {
	...
	for _, ins := range instructions {
		...
		case driver.CTI:
			if pt, err := newParallelTransfer(idGen, vlh, ins, timeEstimate, cumulativeTimeEstimate); err != nil {
				return nil, err
			} else {
		...
		for _, output := range ins.Outputs {
			...
			if content, ok := lastUpdate[wellLocation{DeckItemID: pl.ID, Row: pl.Coords.Y, Column: pl.Coords.X}]; ok {
				...
				if well := vlh.GetWellAt(pl); well != nil {
					well.Contents(idGen).SetName(output.MeaningfulName())
				}
			}
		}
	}
