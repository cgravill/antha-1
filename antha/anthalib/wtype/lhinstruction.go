...
func (ins *LHInstruction) DupLiquids(idGen *id.IDGenerator) {
	for i := 0; i < len(ins.Inputs); i++ {
		ins.Inputs[i] = ins.Inputs[i].Dup(idGen)
	}
	for i := 0; i < len(ins.Outputs); i++ {
		ins.Outputs[i] = ins.Outputs[i].Dup(idGen)
	}
}
...
func (insts LHInstructions) DupLiquids(idGen *id.IDGenerator) {
	for _, ins := range insts {
		ins.DupLiquids(idGen)
	}
}
