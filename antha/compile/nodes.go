// antha/compiler/nodes.go: Part of the Antha language
...
// the print functionality implemented in compiler.go.
...
//
func (p *compiler) linebreak(line, min int, ws whiteSpace, newSection bool) (printedBreak bool) {
...
func (p *compiler) setComment(g *ast.CommentGroup) {
...
func (p *compiler) identList(list []*ast.Ident, indent bool) {
...
func (p *compiler) exprList(prev0 token.Pos, list []ast.Expr, depth int, mode exprListMode, next0 token.Pos) {
	if len(list) == 0 {
		return
	}
...
func (p *compiler) parameters(fields *ast.FieldList) {
...
func (p *compiler) signature(params, result *ast.FieldList) {
	if params != nil {
		p.parameters(params)
	} else {
...
func (p *compiler) isOneLineFieldList(list []*ast.Field) bool {
	if len(list) != 1 {
...
func (p *compiler) setLineComment(text string) {
	p.setComment(&ast.CommentGroup{List: []*ast.Comment{{Slash: token.NoPos, Text: text}}})
}

func (p *compiler) fieldList(fields *ast.FieldList, isStruct, isIncomplete bool) {
...
//	   and 4 (never use spaces) in Compact mode.
//
func (p *compiler) binaryExpr(x *ast.BinaryExpr, prec1, cutoff, depth int) {
...
func (p *compiler) expr1(expr ast.Expr, prec1, depth int) {
	p.print(expr.Pos())
...
func (p *compiler) expr(x ast.Expr) {
	const depth = 1
	p.expr1(x, token.LowestPrec, depth)
}
...
func (p *compiler) stmtList(list []ast.Stmt, nindent int, nextIsRBrace bool) {
	if nindent > 0 {
		p.print(indent)
	}
	var line int
...
func (p *compiler) block(b *ast.BlockStmt, nindent int) {
...
func (p *compiler) controlClause(isForStmt bool, init ast.Stmt, expr ast.Expr, post ast.Stmt) {
	p.print(blank)
	needsBlank := false
...
//
func (p *compiler) indentList(list []ast.Expr) bool {
...
func (p *compiler) stmt(stmt ast.Stmt, nextIsRBrace bool) {
	p.print(stmt.Pos())
...
func (p *compiler) valueSpec(s *ast.ValueSpec, keepType bool) {
	p.setComment(s.Doc)
...
//
func (p *compiler) spec(spec ast.Spec, n int, doIndent bool) {
	switch s := spec.(type) {
	case *ast.ImportSpec:
		p.setComment(s.Doc)
		if s.Name != nil {
...
//
func (p *compiler) nodeSize(n ast.Node, maxSize int) (size int) {
	// nodeSize invokes the compiler, which may invoke nodeSize
...
func (p *compiler) bodySize(b *ast.BlockStmt, maxSize int) int {
	pos1 := b.Pos()
	pos2 := b.Rbrace
...
//
func (p *compiler) adjBlock(headerSize int, sep whiteSpace, b *ast.BlockStmt) {
	if b == nil {
		return
	}
...
func (p *compiler) distanceFrom(from token.Pos) int {
...
func (p *compiler) funcDecl(d *ast.FuncDecl) {
	p.setComment(d.Doc)
...
func (p *compiler) decl(decl ast.Decl) {
	switch d := decl.(type) {
	case *ast.BadDecl:
...
func (p *compiler) declList(list []ast.Decl) {
	tok := token.ILLEGAL
	for _, d := range list {
		prev := tok
		tok = declToken(d)
