...
// A pmode value represents the current compiler mode.
type pmode int
...
type commentInfo struct {
	...
	commentOffset  int               // = compiler.posFor(compiler.comments[cindex].List[0].Pos()).Offset; or infinity
...
type compiler struct {
	...
	mode        pmode        // current compiler mode
...
func (p *compiler) commentsHaveNewline(list []*ast.Comment) bool {
	// len(list) > 0
...
//
func (p *compiler) commentBefore(next token.Position) bool {
...
//
func (p *compiler) commentSizeBefore(next token.Position) int {
...
// or comments.
//
func (p *compiler) recordLine(linePtr *int) {
	p.linePtr = linePtr
}
...
//
func (p *compiler) linesFrom(line int) int {
	return p.out.Line - line
}

func (p *compiler) posFor(pos token.Pos) token.Position {
	// not used frequently enough to cache entire token.Position
	return p.fset.Position(pos)
}

func (p *compiler) lineFor(pos token.Pos) int {
	if pos != p.cachedPos {
		p.cachedPos = pos
...
func (p *compiler) atLineBegin(pos token.Position) {
...
func (p *compiler) writeByte(ch byte, n int) {
	if p.out.Column == 1 {
		p.atLineBegin(p.pos)
	}
...
//
// Note: writeString is only used to write Go/Antha tokens, literals, and
// comments, all of which must be written literally. Thus, it is correct
// to always set isLit = true. However, setting it explicitly only when
// needed (i.e., when we don't know that s contains no tabs or line breaks)
// avoids processing extra escape characters and reduces run time of the
// compiler benchmark by up to 10%.
//
func (p *compiler) writeString(pos token.Position, s string, isLit bool) {
	if p.out.Column == 1 {
		p.atLineBegin(pos)
	}
	...
	if isLit {
		...
		// unchanged. Note that valid Go/Antha programs cannot contain
	...
	for i := 0; i < len(s); i++ {
		// Antha tokens cannot contain '\f' - no need to look for it
		if s[i] == '\n' {
			nlines++
			li = i
		}
...
//
func (p *compiler) writeCommentPrefix(pos, next token.Position, prev, comment *ast.Comment, tok token.Token) {
	if len(p.output) == 0 {
...
// (only tabs and blanks can appear in the compiler's context).
//
...
// contents are nicely laid out after re-printing each line using the compiler's
// current indentation.
//
...
func (p *compiler) writeComment(comment *ast.Comment) {
	text := comment.Text
...
// buffer.
//
func (p *compiler) writeCommentSuffix(needsLinebreak bool) (wroteNewline, droppedFF bool) {
	for i, ch := range p.wsbuf {
		switch ch {
		case blank, vtab:
...
//
func (p *compiler) intersperseComments(next token.Position, tok token.Token) (wroteNewline, droppedFF bool) {
...
func (p *compiler) writeWhitespace(n int) {
	// write entries
	for i := 0; i < n; i++ {
...
//
func (p *compiler) print(args ...interface{}) {
...
// buffer.
//
func (p *compiler) flush(next token.Position, tok token.Token) (wroteNewline, droppedFF bool) {
...
func (p *compiler) printNode(node interface{}) error {
...
unsupported:
	return fmt.Errorf("antha/compiler: unsupported node type %T", node)
}
...
func (cfg *Config) fprint(output io.Writer, fset *token.FileSet, node interface{}, nodeSizes map[ast.Node]int) (lineMap map[int]int, err error) {
	...
	// write compiler result via tabwriter/trimmer to output
